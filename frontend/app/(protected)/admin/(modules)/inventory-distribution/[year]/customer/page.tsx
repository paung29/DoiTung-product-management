...
export default async function Page({
  ...
  const stockTableResponse = await fetch(
    `${baseUrl}/stocks/get-customer-stock-by-year?year=${year}`,
    {
      credentials: "include",
      method: "GET",
      headers: {
  ...
  const stockTableapiData: CustomerStockTableResponse = stockTableResponse.ok
    ? await stockTableResponse.json()
    : { customer_stock_table: [] };

  const records: CustomerHistoryData[] =
    stockTableapiData.customer_stock_table.map((item, index) => ({
  ...
  return <CustomerHome records={records} />;
}
