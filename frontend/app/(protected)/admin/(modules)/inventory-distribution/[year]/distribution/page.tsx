...
export default async function Page({
  ...
  const wareHouserecords: Option[] = wareHouseapiData.warehouses.map(
    (item, index) => ({
  ...
  const customerResponse = await fetch(
    `${baseUrl}/customers/get-all-customers`,
    {
      credentials: "include",
      method: "GET",
      headers: {
  ...
  const customerApiData: CustomersApiResponse = customerResponse.ok
    ? await customerResponse.json()
    : { customers: [] };

  const customerOptions: Option[] = customerApiData.customers.map(
    (item, index) => ({
      id: String(item.id),
  ...
  return (
    <DistributionPage
      years={apiData}
      plantationAreaOptions={wareHouserecords}
