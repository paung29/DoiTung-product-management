...
function PodRecordingForm({ record }: { record: GetPodApiResponse }) {
  ...
  const remainingPods = Math.max(
    Number(record.numberPods || 0) - Number(lostPods || 0),
    0,
  );
      ...
      <form className="flex flex-col">
        ...
        <div className="pb-8">
          <FormCard>
            ...
            <div className="flex flex-col justify-between gap-10 py-4 md:flex-row">
              ...
              <StaffDisable
                title={"Pole-No"}
                placeholder={String(record.poleNo)}
              />
              <StaffDisable
                title={"Cluster-No"}
                placeholder={String(record.clusterNo)}
              />
            </div>
          </FormCard>
        </div>
        ...
        <div className="pb-8">
          <FormCard>
            ...
            {/* Number of Pods Formed and Lost Pods - 2 columns on desktop, 1 on mobile */}
            <div className="grid grid-cols-1 gap-10 py-2 md:grid-cols-2">
              {/* Number of Pods Formed */}
              <div>
                <StaffDisable
                  title={"Number of Pods Formed"}
                  placeholder={String(record.numberPods)}
                />
              </div>
            ...
            {/* Remaining Pods - Full width below */}
            <div className="py-2">
              <div>
