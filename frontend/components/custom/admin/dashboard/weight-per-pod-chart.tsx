 ...
 */
export function WeightPerPodChart({ year }: { year?: string | number }) {
  ...
  // Render the trend whenever there are any years, even if all values are 0,
  // so the chart accurately reflects the backend (0 g points included).
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Yearly trend of average pod weight across production seasons.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex min-h-96 w-full items-center justify-center rounded-xl border border-dashed border-gray-200 bg-gray-50 text-sm text-gray-600">
          No average pod weight data is available yet.
        </div>
      ) : (
        <ChartContainer config={chartConfig} className="min-h-96 w-full">
          ...
          >
            ...
            <YAxis
              ...
              label={{
                value: `Average Weight (${WEIGHT_UNIT})`,
                angle: -90,
                position: "insideLeft",
